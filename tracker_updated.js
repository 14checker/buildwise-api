...
const REQUIRE_APPROVED_SOURCE = String(process.env.REQUIRE_APPROVED_SOURCE || "false").toLowerCase() === "true";
...
async function scrapeTarget(db, target, runData) {
  ...
  const permission = governance.evaluateScrapePermission(db, {
    url: scrapeUrl,
    retailer_id: offer.retailer_id,
    request_type: "pricing",
    requireApprovedSource: REQUIRE_APPROVED_SOURCE
  });

  if (!permission.allowed) {
    governance.logCompliance(db, {
      source_id: permission.source?.source_id || null,
      retailer_id: offer.retailer_id,
      url: scrapeUrl,
      action: "scrape_blocked",
      status: "blocked",
    ...
    const error = core.applyScrapeFailure(db, {
      ...
      retailer_id: offer.retailer_id,
      error_type: `compliance_${permission.reason}`,
      message: `Compliance/source governance blocked scrape: ${permission.reason}. URL=${scrapeUrl || ""}`,
    ...
    runData.skippedOffers.push({
      ...
      retailer_id: offer.retailer_id,
      reason: `compliance_${permission.reason}`,
      url: scrapeUrl || ""
    });

    console.warn(`⚠️ Compliance blocked ${offer.retailer_offer_id}: ${permission.reason}`);
    return { status: "skipped", error };
  }

  governance.logRequest(db, {
    source_id: permission.source?.source_id || null,
    retailer_id: offer.retailer_id,
    url: scrapeUrl,
    request_type: "pricing",
...
async function runAll() {
  ...
  const runData = {
    ...
    summary: {
      ...
      require_approved_source: REQUIRE_APPROVED_SOURCE,
  ...
  console.log(`Require approved source: ${REQUIRE_APPROVED_SOURCE}`);
